              ...
              <div className="w-full space-y-2">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button
                    ...
                    className={`min-w-[80px] transition-all duration-200 ${copied ? 'bg-green-600 hover:bg-green-700' : ''
                      }`}
                  >
                    {copied ? (
                      <>
            ...
            {attestationStatus && (
              <div className="space-y-3">
                ...
                {attestationStatus.twfido && (
                  <div className={`border rounded-lg p-4 ${attestationStatus.twfido.exists
                    ? 'border-green-200 bg-green-50 dark:bg-green-900/20 dark:border-green-800'
                    : 'border-gray-200 bg-gray-50 dark:bg-gray-800 dark:border-gray-700'
                    }`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-medium text-sm">Twfido (自然人憑證)</h4>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${attestationStatus.twfido.exists
                    ...
                    {attestationStatus.twfido.exists && attestationStatus.twfido.data && (
                      <div className="space-y-2">
                        <div className="text-xs font-mono p-2 bg-white dark:bg-gray-900 rounded border">
                          ...
                          <div className="break-all mb-2">{attestationStatus.twfido.data.credentialReference}</div>
                          {attestationStatus.twfido.expiry && (
                            <>
                              <div className="text-gray-500 dark:text-gray-400 mb-1">過期時間:</div>
                              <div className="text-xs">
                                {new Date(attestationStatus.twfido.expiry * 1000).toLocaleString('zh-TW')}
                              </div>
                            </>
                          )}
                        </div>
                ...
                {attestationStatus.twland && (
                  <div className={`border rounded-lg p-4 ${attestationStatus.twland.exists
                    ? 'border-green-200 bg-green-50 dark:bg-green-900/20 dark:border-green-800'
                    : 'border-gray-200 bg-gray-50 dark:bg-gray-800 dark:border-gray-700'
                    }`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-medium text-sm">Twland (房產憑證)</h4>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${attestationStatus.twland.exists
                        ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100'
                        : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'
                        }`}>
                      ...
                      <div className="space-y-2">
                        {attestationStatus.twland.attestations.map((attestation, index) => (
                          <div key={index} className="text-xs font-mono p-2 bg-white dark:bg-gray-900 rounded border">
                            <div className="text-gray-500 dark:text-gray-400 mb-1">房產 #{index + 1} 鏈上地址:</div>
                            ...
                            <div className="text-gray-500 dark:text-gray-400 mb-1">過期時間:</div>
                            <div className="text-xs">
                              {new Date(attestation.expiry * 1000).toLocaleString('zh-TW')}
                            </div>
                          </div>
                        ))}
                      </div>
